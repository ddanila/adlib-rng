...
void display_init(uint32_t seed) {
    ...
    display_vga_puts  (0,  0, ATTR_TITLE, "adlib-rng");
    display_vga_printf(0, 24, ATTR_LABEL, "seed=");
    display_vga_printf(0, 29, ATTR_VALUE, "0x%08lX", (unsigned long)seed);

    display_vga_puts(2,  0, ATTR_LABEL, "Bar:");
    display_vga_puts(2, 16, ATTR_LABEL, "Step:");
    display_vga_puts(2, 32, ATTR_LABEL, "Chord:");
    display_vga_puts(4,  0, ATTR_LABEL, "Mel:");
    display_vga_puts(4, 14, ATTR_LABEL, "Bass:");
    display_vga_puts(4, 28, ATTR_LABEL, "Drum:");

    display_vga_puts(6, 0,  ATTR_LABEL, "Current bar (64 steps):");
    display_vga_putc(7, 0,  ATTR_LABEL, '[');
    display_vga_putc(7, 65, ATTR_LABEL, ']');
    /* Step ruler row 8: tick marks every 8 steps */
    for (i = 0; i < 64; i++) {
        char c = ' ';
        if ((i & 15) == 0) c = '|';
        else if ((i & 7) == 0) c = '.';
        display_vga_putc(8, 1 + i, ATTR_BARLINE, c);
    }

    display_vga_puts(10, 0, ATTR_LABEL, "Bars:");
    for (i = 0; i < BARS; i++) {
        display_vga_putc  (10, 6 + i * 4, ATTR_LABEL,  '[');
        display_vga_printf(10, 7 + i * 4, ATTR_NORMAL, "%02d", i + 1);
        display_vga_putc  (10, 9 + i * 4, ATTR_LABEL,  ']');
    }

    {
        int cur = music_get_variation();
        display_vga_puts(12, 0, ATTR_LABEL, "Variations:");
        for (i = 0; i < NUM_VARIATIONS; i++) {
            int col = 13 + i * 17;
            unsigned char a  = (i == cur) ? ATTR_CURBAR : ATTR_NORMAL;
            unsigned char ab = (i == cur) ? ATTR_CURBAR : ATTR_LABEL;
            display_vga_printf(12, col,     ab, "[%d]", i + 1);
            display_vga_printf(12, col + 4, a,  "%-12s", music_variation_name(i));
        }
        display_vga_puts  (14, 0, ATTR_LABEL, "Now:");
        display_vga_printf(14, 5, ATTR_VALUE, "%-73s", music_variation_desc(cur));
    }

    display_vga_puts(16, 0, ATTR_LABEL,  "Always:");
    display_vga_puts(17, 2, ATTR_NORMAL, "A major pentatonic only (no 4th/7th); melody key-anchored to A");
    display_vga_puts(18, 2, ATTR_NORMAL, "120 BPM, 64 substeps/bar, 12 bars then loop forever");
    display_vga_puts(19, 2, ATTR_NORMAL, "Drums via OPL2 rhythm mode. Pattern varies per variation.");
     ...
     * matches the currently-active seed (if any). */
    {
        int col = 7;
        int j;
        display_vga_puts(21, 0, ATTR_LABEL, "Seeds:");
        for (j = 0; j < SEED_COUNT; j++) {
            ...
            display_vga_printf(21, col,     ab, "[%c]", 'a' + j);
            display_vga_printf(21, col + 3, a,  "%s ", SEED_LABEL[j]);
            col += 3 + (int)strlen(SEED_LABEL[j]) + 2;
        }
    }

    display_vga_puts(24, 0, ATTR_LABEL, "ESC: quit    1-4: variation    a-f: seed");
}
...
static void draw_step_grid(int cur_step, const bar_t *bar) {
    ...
    for (i = 0; i < STEPS_PER_BAR; i++) {
        ...
        display_vga_putc(7, 1 + i, attr, ch);
    }
}
...
static void draw_bar_index(int cur_bar) {
    ...
    for (i = 0; i < BARS; i++) {
        ...
        char buf[3];
        sprintf(buf, "%02d", i + 1);
        display_vga_putc(10, 6 + i * 4, ab, '[');
        display_vga_putc(10, 7 + i * 4, a,  buf[0]);
        display_vga_putc(10, 8 + i * 4, a,  buf[1]);
        display_vga_putc(10, 9 + i * 4, ab, ']');
    }
}
...
void display_frame(int cur_bar, int cur_step, const bar_t *bar) {
    ...
    display_vga_printf(2,  5, ATTR_VALUE, "%2d/12", cur_bar + 1);
    display_vga_printf(2, 22, ATTR_VALUE, "%2d/64", cur_step);
    display_vga_printf(2, 39, ATTR_VALUE, "%-3s",   c);

    display_vga_printf(4,  5, ATTR_MEL,  "%-3s",  m);
    display_vga_printf(4, 20, ATTR_BASS, "%-3s",  b);
    display_vga_putc(4, 34, (d & DRUM_KICK)  ? ATTR_KICK  : ATTR_BARLINE,
                            (d & DRUM_KICK)  ? 'K' : '.');
    display_vga_putc(4, 35, (d & DRUM_SNARE) ? ATTR_SNARE : ATTR_BARLINE,
                            (d & DRUM_SNARE) ? 'S' : '.');
    display_vga_putc(4, 36, (d & DRUM_HAT)   ? ATTR_HAT   : ATTR_BARLINE,
                            (d & DRUM_HAT)   ? 'H' : '.');
